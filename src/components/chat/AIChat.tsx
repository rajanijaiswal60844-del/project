...
import { Loader2, Send, Paperclip, X, User, Volume2, Image as ImageIcon } from 'lucide-react';
import { Avatar } from '../ui/avatar';
import { useToast } from '@/hooks/use-toast';
import Image from 'next/image';
import { aiChatWithGemini } from '@/ai/flows/ai-chat-with-gemini';
import { textToSpeech } from '@/ai/flows/text-to-speech-flow';
import { textToImage } from '@/ai/flows/text-to-image-flow';
...
export default function AIChat() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const isImageGenerationRequest = /^(generate|create|draw an?)\s(image|picture)/i.test(currentInput);

      if (isImageGenerationRequest && !currentImage) {
        const result = await textToImage(currentInput);
    ...
    <div className="flex flex-col h-full border rounded-lg">
      <ScrollArea className="flex-1 p-4" ref={scrollAreaRef}>
        <div className="space-y-6 max-w-4xl mx-auto">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground pt-16 gap-4 text-center">
              <div className="flex items-center gap-2">
                <BotIcon className="w-8 h-8" />
                <ImageIcon className="w-8 h-8" />
              </div>
              <p>Ask me anything, or try "draw a picture of a robot developer".</p>
            </div>
          )}
              ...
              <div className={`flex flex-col gap-2 ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`rounded-lg px-4 py-2 inline-block max-w-[90%] ${message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                    ...
                    {message.generatedImage && (
                       <div className="relative w-64 h-64 mt-2 rounded-md overflow-hidden">
                           <Image src={message.generatedImage} alt="Generated by AI" layout="fill" objectFit="cover" />
                       </div>
                    )}
                </div>
                {message.role === 'bot' && (
